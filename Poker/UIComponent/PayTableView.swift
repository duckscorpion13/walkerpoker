...
class PayTableView : UIView {
	var m_borderColor	= UIColor.white
	...
	func textAttributesForRowHeight(_ height: CGFloat) -> [NSAttributedString.Key : AnyObject] {
		...
		textAttributes = [ .font : font, .foregroundColor : self.m_borderColor, .shadow : textShadow ]
	...
	override func draw(_ rect: CGRect) {
		...
		payColumnWidth = floor(self.bounds.width * 2.0 / 3.0 / 6.0)
		...
		for category in Hand.Category.WinningCategories {
			let catString = category.description
			var textFrame = descFrame.intersection(rowFrame)
			var textSize	= catString.size(withAttributes: textAttrs)
			
			textAttrs[.foregroundColor] = category == self.category ? UIColor.white : self.m_borderColor
			...
			for column in 0 ..< 5 {
				...
				payAttrs[.foregroundColor] = column + 1 == self.bet && category == self.category ? UIColor.white : self.m_borderColor
